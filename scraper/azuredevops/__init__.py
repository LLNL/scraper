...
class AzureDevOpsClient:
    ...
    def _get_all_collections(self) -> List[AzureDevOpsCollection]:
        ...
        if self.is_cloud_ado:
                ...
                if accounts_response.status_code == 200:
                    ...
                    if accounts_json.get('value') and len(accounts_json['value']) > 0:
                        for org in accounts_json['value']:
                            collections.append(AzureDevOpsCollection(
                                id=org['accountId'],
                                name=org['accountName'],
                                url=f"https://dev.azure.com/{org['accountName']}"
                            ))
                            logger.debug(f"Found cloud organization: {org['accountName']}")
                    else:
                    ...
                    raise RuntimeError(f"Failed to retrieve organizations. Status Code: {accounts_response.status_code} Response: {accounts_response.text}")
            else:
                logger.warning(f"Failed to retrieve user profile: {profile_response.status_code} Response: {profile_response.text}")
                logger.warning("Falling back to base address for organization extraction.")
                # Fallback: Try to extract organization from baseAddress
                org_name = self.baseurl.rstrip('/').split('/')[-1]
                if org_name and org_name != "dev.azure.com":
                    collections.append(AzureDevOpsCollection(
                    ...
                    raise RuntimeError("Could not determine organization. Please specify organization in the baseurl.")
        else:
            ...
            if collections_response.status_code == 200:
                ...
                raise RuntimeError(f"Failed to retrieve collections. Status Code: {collections_response.status_code} Response: {collections_response.text}")
    ...
    def _get_all_projects(self, collections: List[AzureDevOpsCollection] = None) -> List[AzureDevOpsProject]:
        """
        ...
        for collection in collections:
            collection_url = f"https://dev.azure.com/{collection.name}" if self.is_cloud_ado else f"{self.baseurl}/{collection.name}"
            ...
            while has_more_projects:
                ...
                if response.status_code != 200:
                    raise RuntimeError(f"Failed to get projects: {response.status_code}")
                ...
                for project in result.get('value', []):
                    ...
                    project_web_url = self._get_web_url_from_api_url(project_api_url, project_name)
                    repo_web_url = self._get_repo_web_url(project_api_url, project_name)
                    
                    projects.append(AzureDevOpsProject(
